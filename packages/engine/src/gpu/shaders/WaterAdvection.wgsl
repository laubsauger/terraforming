...
const MIN_WATER_DEPTH = 0.0001;  // Threshold below which water is considered 0
const MIN_FLOW_DEPTH = 0.001;    // Minimum depth to allow flow (prevents rounding losses)
const VISCOSITY = 0.0;            // No viscosity - free flow
const MAX_FLOW_SPEED = 500.0;    // Maximum flow speed from FlowVelocity shader
const FLOW_TRANSFER_RATE = 0.1; // Conservative transfer rate for stability
const OCEAN_LEVEL = 0.0;        // Sea level height (terrain units)
const OCEAN_DRAINAGE_RATE = 0.95; // Drain 95% of water per frame when at ocean
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  var current_depth = textureLoad(waterDepthTexIn, coord).r;

  // If depth is below minimum, treat as exactly zero to prevent rounding errors
  ...
  if (current_depth > 0.001) { // Lower threshold
    // Add pressure-driven flow based on depth
    // Deeper water = more pressure = faster flow
    // Conservative boost for stability
    let pressure_boost = min(current_depth * 20.0, 1.0); // Up to 2x speed for deep water
  ...
  // Much more conservative scaling to prevent instability
  let depth_factor = min(current_depth * 50.0, 1.0);
  let velocity_scale = mix(0.01, 0.05, depth_factor); // Much lower scale for stability
  ...
  // Very conservative advection to completely prevent oscillation
  // Much lower values for maximum stability
  let flow_normalized = min(flow_speed / 50.0, 1.0); // Less sensitive to flow speed
  let advection_strength = mix(0.1, 0.3, flow_normalized); // 10% to 30% advection - very stable
  ...
  // Ocean drainage: consume water that reaches sea level or below
  let terrain_height = textureLoad(heightTex, coord, 0).r;
  if (terrain_height <= OCEAN_LEVEL + 0.1) { // Near or below ocean level
    // Drain water into the ocean - it flows away
    advected_depth = advected_depth * (1.0 - OCEAN_DRAINAGE_RATE);
  }

  // Ensure non-negative and apply minimum threshold
  // Only zero out if truly negligible
